...
if __name__ == "__main__":
    ...
    writer = SummaryWriter("plots-"+args.env+"/run3")
    outFile = open("Output_Records3.txt", write_mode) # to store the terminal output records to file
    print(net)
    ...
    while True: 
        ...
        if reward is not None: # it returns a non-None value only when the episode ends
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                torch.save(net.state_dict(), "agent/run3/"+args.env+"-best.dat")
            ...
            if games_played==1 or games_played%50==0:
                torch.save(net.state_dict(), "agent/run3/history/After-"+str(games_played)+"-games.dat")
